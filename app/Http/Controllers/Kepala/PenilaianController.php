...
class PenilaianController extends Controller
{
    ...
    public function cetakPkg(Request $request, $guruId)
    {
        ...
        $data = [
            ...
            'aspects' => $penilaians->map(function ($p) {
                return [
                    'aspek' => $p->aspek,
                    'data_penilaian' => $p->data_penilaian,
                    'total_skor' => $p->total_skor,
                    'rata_rata' => $p->rata_rata,
                ];
            })->toArray(),
        ];

        $jsonPath = storage_path('app/pkg_data_' . $guruId . '_' . ($mapelId ?? 'all') . '.json');
        if (!file_put_contents($jsonPath, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
            \Log::error("Cetak PKG Gagal: Tidak dapat membuat file JSON di $jsonPath");
            return redirect()->back()->with('error', 'Gagal memproses data cetak.');
        }

        $templatePath = base_path('resources/contoh_template/Template PKG.docx');
        $outputPdf = storage_path('app/pkg_' . $guruId . '_' . ($mapelId ?? 'all') . '.pdf');

        if (!file_exists($templatePath)) {
            \Log::error("Cetak PKG Gagal: File template tidak ditemukan di $templatePath");
            return redirect()->back()->with('error', 'Template PKG tidak ditemukan.');
        }

        $script = base_path('resources/contoh_template/generate_pkg.ps1');
        $command = "powershell -NoProfile -ExecutionPolicy Bypass -File \"{$script}\" -templatePath \"{$templatePath}\" -outputPath \"{$outputPdf}\" -jsonPath \"{$jsonPath}\"";
        ...
        if ($returnVar !== 0 || !file_exists($outputPdf)) {
            \Log::error("Cetak PKG Gagal (PowerShell): " . implode("\n", $output));
            return redirect()->back()->with('error', 'Gagal menghasilkan PDF. Silakan cek log server.');
        }
